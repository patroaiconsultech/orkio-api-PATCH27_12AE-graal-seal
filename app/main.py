...
useEffect(() => {
  ...
  async function loadMessages(tid, opts = {}) {
    ...
    try {
      const { data } = await apiFetch(`/api/messages?thread_id=${encodeURIComponent(targetId)}&include_welcome=0`, { token, org: tenant });
      ...
      const sameActiveThread = activeThreadIdRef.current === targetId;
      if (force || (sameRequest && sameRequestedThread && sameActiveThread)) {
  ...
  useEffect(() => {
    ...
    clearTmpAssistantDrafts();
    setMessages([]);
    void loadMessages(threadId, { force: true });
  }, [threadId]);
